...
@export var wall_damage_min_speed_mps: float = 5.0 # Снизил до 5, чтобы урон был заметнее
...
func _physics_process(delta: float) -> void:
	...
	var speed_kmh: int = speed_mps * 3.6 # В Godot 3D лучше использовать 3.6
	...
	if has_node("Light_Right"): $Light_Right.visible = Input.is_key_pressed(KEY_S)
	if has_node("Light_Left"): $Light_Left.visible = Input.is_key_pressed(KEY_S)
...
func _on_kill_zone_body_entered(body: Node3D) -> void:
	...
	if body.is_in_group("zombie"):
		take_damage(5.0) # Машина получает урон
		if speed_kmh >= 40:
	...
	if body != self and not body.is_in_group("player"):
		if speed_mps > wall_damage_min_speed_mps:
			...
			print("УДАР! Урон: ", round(damage_to_car))
